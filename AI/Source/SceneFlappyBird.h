...
protected:
	...
	const int iNumBirds = 200;
	int m_objectCount;
	int m_liveObjectCount;
	int m_noGrid;
	float m_gridSize;
	float m_gridOffset;
	...
	bool bResetiRandSeed;
	int iSimulationSpeed = 1;
	const float fMinBias = -3.0f;
	const float fMaxBias = 3.0f;
	const int iNumBirdsToKeep = 1;	// The number of birds to keep unmutated when calling Restart()
};
