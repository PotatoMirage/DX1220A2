...
void SceneHex::Init()
{
	...
	// Exercise Week 13
	//4.	In SceneHex::Init(), initialise the hex map by calculating the dimensions and offsets of a single hexagonal cell
	m_noGrid = 12;
...
void SceneHex::DFS(MazePt curr)
{
	...
	// Exercise Week 13
	//6.	DFS(MazePt curr) - add these codes for alternate LEFT and RIGHT directions.  Can you relate these pseudo codes with the implementation in SceneHex::DFS()?
...
bool SceneHex::BFS(MazePt start, MazePt end)
{
	...
	while (!m_queue.empty())
	{
		...
		// Exercise Week 13
		//7.	Implement BFS(MazePt start, MazePt end) method�s LEFT and RIGHT directions.Can you relate these pseudo codes with the implementation in SceneHex::BFS() ?
		//what the indices of the next position is going to be depends on whether we're at an even or odd column
		//LEFT ALT
		if (curr.x % 2 == 0) //even
		{
	...
	for (int row = 0; row < m_noGrid; ++row)
	{
		for (int col = 0; col < m_noGrid; ++col)
		{
			...
			modelStack.Translate(m_gridOffset + 0.75f * m_gridSize * col,
								 yScale * (m_gridOffset + m_gridSize * row + oddOffset * m_gridSize * 0.5f), 0);
